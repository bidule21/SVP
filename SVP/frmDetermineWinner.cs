    ...
    public partial class frmDetermineWinner : Form
    {
        ...
        private void frmDetermineWinner_Load(object sender, EventArgs e)
        {
            using (SVPEntitiesContainer context = new SVPEntitiesContainer())
            {
                ...
                else
                {
                    ...
                    int place = 1;
                    foreach (var sequence in context.Prices.Include("Sequences.Member").Include("Sequences.Shots").FirstOrDefault(x => x.Id == price.Id).Sequences.OrderBy(x => x.NextSequence == null ? 0 : x.NextSequence.Shots.Sum(y => y.Value)).ThenBy(x => x.Shots.Sum(y => y.Value)))
                    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            using (SVPEntitiesContainer context = new SVPEntitiesContainer())
            {
                ...
                {
                    reevaluateFrm = new frmReevaluate(price, context.Sequences.Where(x => idsToReevaluate.Contains(x.Id)).ToList());
                }
            }
            reevaluateFrm.ShowDialog();
            //ToDo: Reload this Form
        }
    }
}
